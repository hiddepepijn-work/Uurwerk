...
- Model: `JARVIS_MODEL` in de service-unit, nu **`gemini-3.8-flash`** (gratis laag, geen kaart; key via aistudio.google.com). Ook: `gpt-6-luna` (Responses-API,
  via de Responses-API, **nadenken op medium** (`JARVIS_EFFORT`), ~€1–2 per maand). Ook mogelijk: `claude-opus-5`,
