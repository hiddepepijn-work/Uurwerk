/**
 * ★ The backend implementation of TimeTrackerAPI, over Electron IPC. ★
 *
 ...
 * data. Domain rules live in core so the web view can reuse them.
 */
...
import { PublishService, buildIndex } from '@core/services/publish.js'
...
import {
  ...
  type DayProposal,
  type RangeProposal
} from '@core/services/planner/index.js'
...
import { dbPath, reportDir } from './paths.js'
import { log } from './logger.js'
import { hasSecret, setSecret, type SecretKey } from './secrets.js'
...
// Re-exported so the rest of the main process keeps importing it from one place, even
// though it now lives in events.ts to keep capture.ts and timelapse.ts out of a cycle.
export { emitEvent }
...
export function buildImplementation(
  ...
  return {
    ...
    capture: {
      ...
      buildTimelapse: async (date) => buildTimelapse(backend, date)
    },
    ...
    reports: {
      ...
      generateDocx: async (week) => {
        ...
        const directory = reportDir(store.settings.get().reportOutputDir)
    ...
    settings: {
      ...
      setSecret: async (key, value) => setSecret(key as SecretKey, value),
      hasSecret: async (key) => hasSecret(key as SecretKey)
    },
    ...
    startup: {
      getLoginItemStatus: async () => getLoginItemStatus(store.settings.get().autoLaunch),
